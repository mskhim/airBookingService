...
--좌석 출력하는 쿼리문
SELECT S.ROWX,S.COLY,CUSTOMER_NO,GROUP_NO,P.COLY AS YNUM FROM SEATS S LEFT JOIN BOOKING B ORDER BY ROWX, COLY ON S.NO = B.SEATS_NO " + "JOIN PLANE P " + "ON P.NO = S.PLANE_NO WHERE S.PLANE_NO = (SELECT PLANE_NO FROM FLIGHT F JOIN BOOKING B ON flight_no = F.NO;